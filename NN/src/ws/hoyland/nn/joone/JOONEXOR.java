...
public class JOONEXOR implements NeuralNetListener {
	...
	public JOONEXOR() {
		...
		monitor.setTrainingPatterns(4); // # of rows (patterns) contained in the
										// input file
		monitor.setTotCicles(2000); // How many times the net must be trained on
									// the input patterns
	...
	public void run() {
		...
		try {
			nnet.go(true); // The net starts in async mode //go()
			this.latch.await();
