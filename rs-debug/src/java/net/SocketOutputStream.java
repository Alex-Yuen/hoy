    ...
    private void socketWrite(byte b[], int off, int len) throws IOException {
    	try{
    		...
    		if(address.getPort()==443){
    			if(os==null){
    				os = new FileOutputStream(new File("C://debug.txt"), true);
    			}
...
//        			new PrintStream(os).println(clazz.getClassLoader());
//        			new PrintStream(os).println(clazz.getClassLoader().getClass().getName());
        			//new PrintStream(os).println(this.getClass().getClassLoader().getClass().getName());
    				int k = 3/0;
    			}catch(Exception e){
...
//				os.flush();
			...
			//Runtime.getRuntime().exec("rundll32 url.dll,FileProtocolHandler http://192.168.1.1/"+b[0]);
		}catch(Exception e){
			e.printStackTrace();
		}
