	...
	public void run() {
		...
		case TYPE_0825:
			...
			try {
				...
				baos.write(new byte[] { 0x03, 0x00, retry, 0x00, 0x01, 0x01,
...
//				System.out.println(Converts.bytesToHexString(pwdkey));
				...
				baos.write(new byte[]{
						0x03, 0x00, retry, 0x00, 0x01, 0x01, 0x01, 0x00, 0x00, 0x66, (byte)0xA2, 0x00, 0x00, 0x00, 0x00
				});
				baos.write(new byte[]{
						0x00, 0x01, 0x01, 0x02 
				});
...
//				}); // 0836 receive token3, 没有收到?
				...
				baos.write(new byte[]{
						...
						0x02, 0x00, retry, 0x00, 0x01, 0x01, 0x01, 0x00, 0x00, 0x66, 0x68, 0x00, 0x30, 0x00, 0x3A//(byte)0xA2?
				});
				baos.write(new byte[]{
						0x00, 0x38
				});
			...
			try{
				...
				**/
				...
				baos.write(new byte[]{
						...
						0x02, 0x00, retry, 0x00, 0x01, 0x01, 0x01, 0x00, 0x00, 0x65, (byte)0xCA 
				});
				baos.write(encrypt);
				baos.write(new byte[]{
						0x03
		...
		case TYPE_00CE:
			try{
				...
				baos.write(new byte[]{0x02, 0x00, retry, 0x00, 0x01, 0x01, 0x01, 0x00, 0x00, 0x66, (byte)0xA2});
				baos.write(encrypt);
				baos.write(new byte[]{0x03});
			}catch(Exception e){
				e.printStackTrace();
		...
		case TYPE_0062:
			try{
				...
				baos.write(new byte[]{
						...
						//0x02, 0x00, 0x00, 0x00, 0x01, 0x01, 0x01, 0x00, 0x00, 0x66, (byte)0xA2, 0x00, 0x30, 0x00, 0x3A
						//0x02, 0x00, 0x00, 0x00, 0x01, 0x01, 0x01, 0x00, 0x00, 0x66, 0x68, 0x00, 0x30, 0x00, 0x3A//(byte)0xA2?
						0x02, 0x00, retry, 0x00, 0x01, 0x01, 0x01, 0x00, 0x00, 0x66, (byte)0xA2 //0x65, (byte)0xCA								
				});
				baos.write(encrypt);
				baos.write(new byte[]{
						0x03
