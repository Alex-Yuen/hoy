...
public class StyleSheet
{
	...
	public static int parseColorValue(StringUtil tokenizer)
	{
		...
		if(c=='#') // hex
		{
			String hexStr = tokenizer.nextWord().substring(1);
			if(hexStr.length()==3)
		...
		{
			String val = tokenizer.lastToken().toLowerCase().trim();
			if(val.startsWith("rgb"))
			{
				String rgb = val.substring(val.indexOf('('),val.length()-1); // comma seperated values
