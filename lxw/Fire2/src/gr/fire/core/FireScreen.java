...
public class FireScreen extends Canvas implements Runnable
{
	 ...
	 * 
	 */
	public static final int ZINDEX_MAX = 8;
		...
		try
		{
			while (!destroyed)
			{
				...
				if (visible)
				{
					...
					if (!anim.isRunning())// animation completed. remove it
											// from the queue.
					{
						if (anim.parent == null) // top level animations must
					...
					int x = anim.x, y = anim.y, w = anim.width, h = anim.height;// get
																				// coords
																				// and
																				// dimensions
																				// before
					...
					{// i need to clear the old frame of the animation
						if (anim.parent != null && anim.parent.parent != null) // NOT
																				// a
																				// top
																				// level
																				// animation,
	 ...
	 * 
	 * @param display,
	 *            if not null and its the first call of the method, a FireScreen
	 *            instance for this display is created.
