...
/**
 * This is a simple XML to object de-serializer. The <code>SimpleReader</code>
 * class implements <code>ObjectReader</code>. It reads an object from a JDOM
 * element and puts it back to a live Java object. The XML representation of the
 * object is a standard WOX representation. For more information about the XML
 * representation please visit: http://woxserializer.sourceforge.net/
 * 
...
public class SimpleReader extends TypeMapping implements ObjectReader {
	...
	/*
	 * public void printMap(){ Set set = map.entrySet(); Iterator it =
	 * set.iterator(); while(it.hasNext()){ Map.Entry entry =
	 * (Map.Entry)it.next(); //System.out.println("key: " + entry.getKey() + ",
	 * value: " + entry.getValue()); } }
	 */

	/**
	 * This is the only public method of <code>SimpleReader</code>. This
	 * method reads a JDOM element (which represents the object), and returns a
	 * live java object. The JDOM element is the object in a standard XML
	 * representation defined by WOX. This is the low level entry point of the
	 * WOX de-serializer.
	 * 
	 * @param xob
	 *            The JDOM element that represents the object.
	...
	public Object read(Element xob) {
		 ...
		 * ob = readWOXReference(xob, id); } else
		 */
		if (isPrimitiveArray(xob)) {
			// System.out.println("readPrimitiveArray: " +
			// xob.getAttributeValue(TYPE));
			ob = readPrimitiveArray(xob, id);
		} else if (isArray(xob)) {
			// System.out.println("readObjectArray: " +
			// xob.getAttributeValue(TYPE));
...
//			System.out.println("readHashMap: " + xob.getAttributeValue(TYPE));
			...
			// System.out.println("readStringObject: " +
			// xob.getAttributeValue(TYPE));
	...
	private Object readPrimitiveArray(Element xob, Object id) {
		try {
			...
			// Class type =
			// (Class)mapWOXToJava.get(xob.getAttributeValue(TYPE));
			Class type = (Class) mapWOXToJava.get(xob
					.getAttributeValue(ELEMENT_TYPE));
			// get the wrapper type to be able to construct the elements of the
			// array
			Class wrapperType = getWrapperType(type);
			// System.out.println("type: " + type + ", wrapperType: " +
			// wrapperType);

			Constructor cons = null;
			if ((!type.equals(char.class)) && (!type.equals(Character.class))
					&& (!type.equals(Class.class))) {
				// get the constructor for the wrapper class - this will always
				// take a
				// String argument - except Character data type, which takes
				// char value!
			...
			/*
			 * if (type.equals(int.class)) { Object intArray =
			 * readIntArray((int[]) array, xob); return intArray; }
			 */
			/*
			 * if (type.equals(Integer.class)) { Object intArray =
			 * readIntArray((Integer[]) array, xob); return intArray; }
			 */
			...
			while (st.hasMoreTokens()) {
				...
				/*
				 * Character c[] = new Character[]{new Character('a')}; Object
				 * value = null; if
				 * (wrapperType.getName().equals("java.lang.Character")){ c[0] =
				 * new Character(s.charAt(0)); //System.out.println("using
				 * constructor with c: " + c); value = cons.newInstance(c); }
				 * else{ //System.out.println("using constructor with args: " +
				 * args); value = cons.newInstance(args); }
				 */
				// --------------------------------------------------------------
				// Previous code
				// Object value = cons.newInstance(args);
		...
		while (st.hasMoreTokens()) {
			...
			if (s.equals("null")) {
				...
				if (javaClass == null) {
					...
					if (javaClass == null) {
						try {
							// System.out.println("class NOT found in any of the
							// maps: " + s);
						...
						// System.out.println("WOX type: " + s + ", Java type
						// (array): " + javaClass);
						a[index++] = javaClass;
					}
				} else {
	...
	private Object readArrayList(Element xob, Object id) {
		...
		/*
		 * try { //String arrayTypeName = xob.getAttributeValue(TYPE); String
		 * arrayTypeName = xob.getAttributeValue(ELEMENT_TYPE); int len =
		 * Integer.parseInt(xob.getAttributeValue(LENGTH)); Class componentType =
		 * getObjectArrayComponentType(arrayTypeName); Object array =
		 * Array.newInstance(componentType, len); //map.put(id, array); // now
		 * fill in the array List children = xob.getChildren(); int index = 0;
		 * for (Iterator i = children.iterator(); i.hasNext();) {
		 * //System.out.println("before reading..."); Object childArray =
		 * read((Element) i.next()); //System.out.println(index + " child: " +
		 * childArray); Array.set(array, index++, childArray); } ArrayList list =
		 * new ArrayList(); //populate the ArrayList with the array elements for
		 * (int i=0; i<Array.getLength(array); i++){ list.add(Array.get(array,
		 * i)); }
		 * 
		 * map.put(id, array); return array; } catch (Exception e) {
	...
	private Object readObjectArray(Element xob, Object id) {
		 ...
		 * //System.out.println("before reading..."); Object childArray =
		 * read((Element) i.next()); //System.out.println(index + " child: " +
		 * childArray); Array.set(array, index++, childArray); } return array; }
	 ...
	 * 
	 * @param arrayTypeName
	 *            The type of the array as string.
	 * @return The class of the array component.
	 */
	private Class getObjectArrayComponentType(String arrayTypeName)
			throws Exception {
		...
		if (javaClass == null) {
			...
			if (javaClass == null) {
				// System.out.println("WOX type not found in any of the
				// maps...");
	...
	private Object readStringObject(Element xob, Object id) {
		try {
			...
			Class type = (Class) TypeMapping.mapWOXToJava.get(xob
			...
			if (type.equals(Class.class)) {
				...
				Class javaClass = (Class) mapWOXToJava.get(xob
						.getAttributeValue(VALUE));
				// if not found, look for it in the array map
				if (javaClass == null) {
					javaClass = (Class) mapArrayWOXToJava.get(xob
							.getAttributeValue(VALUE));
					// if not found, load it
					if (javaClass == null) {
						// System.out.println("NOT found in any of the arrays: "
						// + xob.getAttributeValue(VALUE));
						Object obClass = Class.forName(xob
								.getAttributeValue(VALUE));
					...
					// if found in the array map
						...
						// System.out.println("Found in the Array Map: " +
						// javaClass);
				...
				// if found in the first map
					...
					// System.out.println("Found in the First Map: " +
					// javaClass);
					map.put(id, javaClass); // added Oct 2006
					return javaClass;
				}
				...
				// xob.getText());
				// if it was a primitive class (i.e. double, boolean, etc.),
				// then get it from the map
				/*
				 * System.out.println("xob.getText()" +
				 * xob.getAttributeValue(VALUE)); Object primitiveClass =
				 * primitivesMap.get(xob.getAttributeValue(VALUE)); if
				 * (primitiveClass != null){ map.put(id, primitiveClass);
				 * //added Oct 2006 return ((Class)primitiveClass); }
				 * //otherwise load the appropriate class and return it //Object
				 * obClass = Class.forName(xob.getText()); Object obClass =
				 * Class.forName(xob.getAttributeValue(VALUE)); map.put(id,
				 * obClass); //added Oct 2006 return obClass;
				 */
			}
			/** *************************************** */
			/*
			 * else if
			 * (type.equals(java.util.concurrent.atomic.AtomicLong.class)){
			 * //System.out.println("it is atomic long..."); Class[] st =
			 * {long.class}; Constructor cons = type.getDeclaredConstructor(st); //
			 * System.out.println("String Constructor: " + cons); Object ob =
			 * makeObject(cons, new Object[]{new Long(xob.getText())}, id);
			 * return ob;
			 *  }
			 */
			/** ***************************************** */
			else {
				// if it is a Character object - special case because Character
				// has no constructor
				// that takes a String. It only has a constructor that takes a
				// char value
				if (type.equals(char.class)) {
					// int decimalValue = getDecimalValue(xob.getText());
					int decimalValue = getDecimalValue(xob
							.getAttributeValue(VALUE));
					...
					return charObject;
					/*
					 * System.out.println("it is CHAR!!!"); st = new
					 * Class[]{char.class}; System.out.println("charText: " +
					 * charText + ", decimalValue: " + );
					 */
				}
				// for the rest of the Wrapper objects - they have constructors
				// that take "String"
				else {
					Object ob = this.makeWrapper(type, xob
							.getAttributeValue(VALUE));
					map.put(id, ob);
					return ob;
					// commented on 16 April 2008 to solve the wrapper problem
					/*
					 * Class[] st = {String.class}; Constructor cons =
					 * type.getDeclaredConstructor(st); //Object ob =
					 * makeObject(cons, new String[]{xob.getText()}, id); Object
					 * ob = makeObject(cons, new
					 * String[]{xob.getAttributeValue(VALUE)}, id); return ob;
					 */
...
// System.out.println("unicodeValue: " + unicodeValue);
		String unicodeModified = unicodeValue.substring(2, unicodeValue
				.length());
	...
	private Object readObject(Element xob, Object id) {
		...
		try {
			...
			/** ********************************************************* */
			// if the TYPE is "java.lang.reflect.Method", this is a special
			// case. We will construct
			// the method by invoking the getMethod method
			// System.out.println("/*********************************************************************/");
			/*
			 * if
			 * (xob.getAttributeValue(TYPE).equals("java.lang.reflect.Method")){
			 * //we get the info about the method String methodName = ""; String
			 * className = ""; Class[] methodParameters = null; for (Iterator i =
			 * xob.getChildren().iterator(); i.hasNext();) { Element fe =
			 * (Element) i.next(); String name = fe.getAttributeValue(NAME); if
			 * (name.equals("clazz")) className = fe.getChild(OBJECT).getText();
			 * else if (name.equals("name")) methodName =
			 * fe.getChild(OBJECT).getText(); else if
			 * (name.equals("parameterTypes")){ System.out.println("getting the
			 * parameter types..."); Element child = (Element)
			 * fe.getChildren().iterator().next(); methodParameters = (Class[])
			 * read(child); } //System.out.println("parameterTypes");
			 * //System.out.println("fe: " + fe); } System.out.println("CLASS: " +
			 * className + ", METHOD: " + methodName);
			 * System.out.println("methodParameters.length: " +
			 * methodParameters.length); for(int g=0; g<methodParameters.length;
			 * g++){ System.out.println("m[" + g + "]: " + methodParameters[g]); }
			 * 
			 * Class myClass = Class.forName(className); Method method =
			 * myClass.getMethod(methodName, methodParameters);
	...
	private void setFields(Object ob, Element xob) {
		...
		for (Iterator i = xob.getChildren().iterator(); i.hasNext();) {
			...
			try {
				...
				// System.out.println("Field name: " + name + " belonging to: "
				// + declaringType);
				...
				if (Util.primitive(field.getType())) {
					...
					// System.out.println("setfield... primitive...type: " +
					// fe.getAttributeValue(TYPE));
					if (fe.getAttributeValue(TYPE).equals("char")) {
						int decimalValue = getDecimalValue(fe
								.getAttributeValue(VALUE));
						Character charObject = new Character(
								(char) decimalValue);
						// System.out.println("decimalvalue: " + decimalValue +
						// ", charObject: " + charObject);
						value = charObject;
					} else {
						value = makeWrapper(field.getType(), fe
								.getAttributeValue(VALUE));
					}
				}
				// it means that the datatype is a Wrapper or a String
				else if (mapWOXToJava.get(fe.getAttributeValue(TYPE)) != null) {
					Class typeWrapper = (Class) TypeMapping.mapWOXToJava.get(fe
							.getAttributeValue(TYPE));
					// if it is a Character object - special case because
					// Character has no constructor
					// that takes a String. It only has a constructor that takes
					// a char value
					...
					// for the rest of the Wrapper objects - they have
					// constructors that take "String"
					else {
						Class[] st = { String.class };
						Constructor cons = typeWrapper
								.getDeclaredConstructor(st);
						cons.setAccessible(true);
						// System.out.println("STEP 2");
						value = cons.newInstance(new String[] { fe
								.getAttributeValue(VALUE) });
						// Object ob = makeObject(cons, new
						// String[]{xob.getText()}, id);
						// Object ob2 = makeObject(cons, new
						// String[]{fe.getAttributeValue(VALUE)});
						// /value = ob2;
						// return ob;
					}
					...
					Element child = (Element) fe.getChildren().iterator()
							.next();
					value = read(child);
				}
	...
	private Object makeObject(Constructor cons, Object[] args, Object key)
			throws Exception {
	...
	private Object makeWrapper(Class type, String value) throws Exception {
		Class wrapperType = getWrapperType(type);
		// System.out.println("wrapperType: " + wrapperType + " : " + type + " :
		// " + (type == int.class));
		Constructor cons = wrapperType
				.getDeclaredConstructor(new Class[] { String.class });
