# Make file for the rabbit/3 project...
...
ifndef JCFLAGS
JCFLAGS := -source 1.5 -encoding ISO-8859-1 -Xlint:unchecked -deprecation
endif
...
ifndef JDFLAGS
JDFLAGS := -windowtitle "RabbIT/3" -doctitle "RabbIT/3" -use -version -author \
-encoding ISO-8859-1 \
...
help:
	@echo " - RabbIT3 somewhat generic Java makefile -"
...
%.jar : 
	@$(COND_COMPILE)
	@echo building $@
	@$(JAR) -cmf Manifest.mf $@ -C $(CLASSDIR) $(JARPACKAGEBASE) Help.txt
...
extras:
	@cat LICENSE > htdocs/LICENSE.txt
	@cat README > htdocs/README.txt
	@cat conf/rabbit.conf > conf/rabbit.conf.orig
